...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, output_1, output_2, ref_1, ref_2, gamma):
        embeddingDist = torch.pow(self.l2norm(output_1, output_2), 2)
        refDist = torch.pow(self.l2norm(self.referenceTransform(ref_1, gamma), self.referenceTransform(ref_2, gamma)), 2)

        lossTensor = torch.pow(embeddingDist - refDist, 2)
        return torch.mean(lossTensor)

    def referenceTransform(self, input, gamma):
        #return torch.pow(input / input + 1, gamma)
        return input